...
school_lookup = {'國小':'ele','國中':'jun','高中':'sen','elementary':'ele','junior':'jun','senior':'sen'}
...
def gsd():
    ...
    school_max_dist = request.args.get('school_dist')
    ...
    if school_type is not None and school_max_dist is None:
        school_max_dist = 300
    ...
    with psycopg.connect("user=postgres password=adbfinals host=adbtest.c9d470mwdvu3.ap-northeast-1.rds.amazonaws.com") as conn:
        ...
        with conn.cursor() as cur:
            query = "SELECT (serial_number,area,building_type,built_date,price,longitude,latitude) FROM property_with_criterias WHERE ST_DWithin(ST_Transform(geom,3826),ST_Transform(stations,3826),%s) AND ST_DWithin(ST_Transform(geom,3826),ST_Transform(parks,3826),%s) AND district like %s "
            params = [mrt_max_dist,park_max_dist,specific_district]
            if(school_type is not None and school_type in school_lookup):
                query = query + "AND ST_DWithin(ST_Transform(geom,3826),ST_Transform("+ school_lookup[school_type] +",3826),%s)"
                params.append(school_max_dist)
            ...
            for record in cur:
                record = record[0]
                tmp = {'Building_id':record[0],'Area':record[1],'Building_type':type_lookup[record[2]],'Built_date':record[3],'price':record[4],'longitude':float(record[5]),'latitude':float(record[6])}
                out.append(tmp)
        conn.commit()
    return jsonify(out)
...
def gllsd():
    ...
    with psycopg.connect("user=postgres password=adbfinals host=adbtest.c9d470mwdvu3.ap-northeast-1.rds.amazonaws.com") as conn:
        with conn.cursor() as cur:
            ...
            for record in cur:
                record = record[0]
                tmp = {'Building_id':record[0],'Area':record[1],'Building_type':type_lookup[record[2]],'Built_date':record[3],'price':record[4],'longitude':float(record[5]),'latitude':float(record[6]),'distance':record[7]}
                out.append(tmp)
        conn.commit()
    return jsonify(out)
